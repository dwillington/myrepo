def call(config) {
	...
	if (com.td.jenkins.util.Utilities.hasRequiredConfig(this, config, required)) {
		switch(config.notification_type) {
			case "EMAIL":
				def emailNotification = new com.td.jenkins.notify.email.EmailNotification(this, config.stage_name)
			...
			case "SCM":
				...
				break
			case "PUBLISH_REPORT":
				def publishReport = new com.td.jenkins.util.PublishReport(this, config.stage_name)
				currentBuild.result = publishReport.run(config)
				break
			default:
