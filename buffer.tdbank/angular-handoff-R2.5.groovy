    ...
    node("CBAW-Main") {
        ...
        stage ('Fetch and Integrate Latest Artifact') {
            withEnv(["PATH+WHATEVER=${tool 'MAVEN 3.3.9 Default - Linux'}/bin"]) {
                ...
                def artifactID="mortgage-preapproval-client"
                ...
                sh """
                    ...
                    cd target
                    unzip mortgage-preapproval-client-*.zip
                    cp -r $WORKSPACE/target/.tmp/pages $WORKSPACE/src
                    cp -r $WORKSPACE/target/.tmp/staticresources $WORKSPACE/src
                    cd ..
                    rm -rf target
                """
            }
        ...
        stage ('Commit Changes') {
            sh """
                #export NAME=\$(find target -name mortgage-preapproval-client-*.zip -type f | cut -d/ -f2 | rev | cut -d. -f2- | rev)
                git add -A
                git commit -am 'HOJ-9672 - Integrating mortgage-preapproval-client into rcp-salesforce' || true
            """
        }
        stage ('Push Changes') {
